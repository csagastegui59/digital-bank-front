...
export const adminAccountService = {
  getBlockedAccounts: async (accessToken: string): Promise<Account[]> => {
    try {
      const response = await fetch(`${API_URL}/admin/accounts/blocked`, {
        method: 'GET',
        headers: {
  ...
  getUnlockRequests: async (accessToken: string): Promise<Account[]> => {
    try {
      const response = await fetch(`${API_URL}/admin/accounts/unlock-requests`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${accessToken}`,
  ...
  searchAccounts: async (query: string, accessToken: string): Promise<Account[]> => {
    try {
      const response = await fetch(`${API_URL}/admin/accounts/search?q=${encodeURIComponent(query)}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${accessToken}`,
